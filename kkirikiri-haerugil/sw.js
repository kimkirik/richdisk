...
const CACHE_NAME = "kkirikiri-pages-shell-v3-accuracy";
const STATIC_ASSETS = [
  `${APP_BASE}/`,
  `${APP_BASE}/calendar/`,
...
self.addEventListener("activate", (event) => {
  event.waitUntil(
    caches.keys()
      .then((keys) => Promise.all(keys.filter((key) => key.startsWith("kkirikiri-pages-shell-") && key !== CACHE_NAME).map((key) => caches.delete(key))))
...
self.addEventListener("fetch", (event) => {
  ...
  if (request.mode === "navigate") {
    event.respondWith(fetch(request).catch(() => caches.match(request).then((cached) => cached || caches.match(url.pathname.startsWith(`${APP_BASE}/calendar`) ? `${APP_BASE}/calendar/` : `${APP_BASE}/`))));
  }
});
