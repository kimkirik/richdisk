...
import { buildAlerts, validSnapshot, day, type Alert } from './alerts.ts';
export type Bindings = { DB: D1Database; VAPID_PUBLIC_KEY: string; VAPID_PRIVATE_KEY: string; APP_URL: string };
...
export async function deliver(env: Bindings, device: Device, event: EventRow) {
  ...
  try {
    const request = await buildPushPayload({ data: { eventId: event.id, deviceId: device.id, title: event.title, body: event.body, url: env.APP_URL,
    ...
    JSON.parse(device.subscription) as PushSubscription, { subject: env.APP_URL, publicKey: env.VAPID_PUBLIC_KEY, privateKey: env.VAPID_PRIVATE_KEY });
...
export async function tick(env: Bindings) {
  ...
  try {
    const snapshot = await getState(env, 'snapshot');
  ...
  const counts = await deliverPending(env);
