...
test('real Worker SQLite, automatic alarms, outage preservation and authenticated push API', {timeout:45000}, async () => {
 ...
 try {
  ...
  assert.equal((await json('/api/profile')).status,401);
  ...
  const profile={householdSize:2,recipient:true,senior:true,disabled:true};
  assert.equal((await json('/api/profile',{method:'POST',headers,body:JSON.stringify({...profile,privateNote:'reject extra fields'})})).status,400);
  assert.equal((await json('/api/profile',{method:'POST',headers,body:JSON.stringify(profile)})).status,200);
  assert.deepEqual((await json('/api/profile',{headers})).data.profile,profile);
  assert.equal((await json('/api/profile',{headers:{...headers,Authorization:'Bearer '+'f'.repeat(64)}})).status,401);
  assert.equal((await json('/api/profile',{method:'DELETE',headers})).status,200);
  assert.equal((await json('/api/profile',{headers})).data.profile,null);
  assert.equal((await json('/api/profile',{method:'POST',headers,body:JSON.stringify(profile)})).status,200);
  ...
  const allIds=(data:any)=>[...data.notices,...data.archived].map((n:any)=>n.id).sort();
  assert.deepEqual(allIds(after),allIds(original.data),'source outage must retain every notice; elapsed deadlines may move notices to archive');
  assert.ok(after.notices.every((n:any)=>!n.closeAt || n.closeAt>=new Date(Date.now()+9*3600000).toISOString().slice(0,10)));
