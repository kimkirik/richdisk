...
import { acknowledge, authDevice, deliver, getState, hash, putEvent, throttle, tick, type Bindings } from '@/lib/service';
const bindings = () => env as unknown as Bindings;
const origins = new Set(['https://kimkirik.github.io', 'http://localhost:5173']);
...
async function handle(request: Request) {
  ...
  try {
    ...
    if (request.method === 'GET' && action === 'config') return reply(request, { publicKey: e.VAPID_PUBLIC_KEY, senderReady: !!e.VAPID_PUBLIC_KEY && !!e.VAPID_PRIVATE_KEY, health: await getState(e, 'health'), platform: 'android', pollMinutes: 30 });
