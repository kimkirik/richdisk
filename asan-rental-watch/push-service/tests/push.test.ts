...
import { buildAlerts, validateSubscription } from '../lib/alerts.ts';
import { acknowledge, deliver, hash, putEvent, throttle, tick, type Bindings } from '../lib/service.ts';
...
function fixture() {
  ...
  db.exec(readFileSync(new URL('../drizzle/0000_strange_king_bedlam.sql', import.meta.url), 'utf8'));
  ...
  const env = { DB: { prepare }, VAPID_PUBLIC_KEY: vapid.getPublicKey().toString('base64url'), VAPID_PRIVATE_KEY: vapid.getPrivateKey().toString('base64url') } as unknown as Bindings;
