const CACHE_NAME = "asan-rental-shell-v5";
const PUSH_API = "https://asan-rental-push.kimkirik.chatgpt.site/api";
...
self.addEventListener("activate", event => {
  event.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(key => key.startsWith("asan-rental-shell-") && key !== CACHE_NAME).map(key => caches.delete(key)))).then(() => self.clients.claim()));
});
...
self.addEventListener("fetch", event => {
  ...
  if (url.origin !== BASE_URL.origin || !url.pathname.startsWith(BASE_URL.pathname) || url.pathname === DATA_URL.pathname) return;
