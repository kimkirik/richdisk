...
import type { LiveNotice as Notice } from "../source/lib/notices.ts";
import { koreanToday } from "../source/lib/notices.ts";
import { reconcileNotices, stableId } from "./reconcile-notices.ts";
import type { ScanResult } from "./reconcile-notices.ts";
...
const DATA_FILE = process.env.ASAN_RENTAL_DATA_FILE || new URL("../data/notices.json", import.meta.url);
...
async function fetchJson<T>(url: string, body: URLSearchParams): Promise<T> {
  ...
  return (await response.json()) as T;
    ...
    if (url !== MYHOME_LIST) throw error;
    // Some hosted runners cannot establish Undici's connection to this portal.
    // Retry the same public endpoint over IPv4 with curl's longer connect timeout.
    // execFile passes arguments directly, with no shell interpolation.
    console.warn("MyHome connection retry via IPv4", String(error));
    const { stdout } = await promisify(execFile)("curl", [
      "--ipv4", "--fail", "--silent", "--show-error", "--connect-timeout", "25", "--max-time", "50",
      ...
      "--user-agent", "Mozilla/5.0 (compatible; AsanRentalWatch/1.0)",
      "--referer", "https://www.myhome.go.kr/hws/portal/sch/selectRsdtRcritNtcView.do",
      "--data-raw", body.toString(), url,
    ], { maxBuffer: 10 * 1024 * 1024, timeout: 165000 });
...
export async function scanLh(): Promise<ScanResult> {
  try {
    ...
    // Bounded concurrency respects official sites while covering every candidate.
...
async function readPrevious() {
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "ENOENT") return { notices: [], archived: [] };
    throw error;
  }
}
