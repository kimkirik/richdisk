...
const channelDiversityWeights = [1, 0.35, 0.15, 0.08, 0.05, 0.035, 0.025, 0.02];
...
function contentRelevanceWeight(title) {
  if (/official\s+(music\s+)?video|music\s+video|뮤직\s*비디오|\bM\/?V\b/i.test(title)) return 0.12;
  if (/티저|teaser|trailer|예고|선공개|preview|coming\s+soon|힌트캠|hint\s*cam/i.test(title)) return 0.25;
  return 1;
}
...
function estimateDailyViews(views, publishedAt, now) {
  ...
  return Math.min(views, ((views * 24) / ageHours) * 0.5);
}

function trendScore(dailyViews, views, publishedAt, title, now) {
  const published = publishedTime(publishedAt, now);
  ...
  const sustainableReach = Math.sqrt(Math.max(0, views) * dailyViews);
  const popularityScore = dailyViews * 0.85 + sustainableReach * 0.15;
  return Math.round(
    popularityScore * recencyWeight * contentRelevanceWeight(title) * 1000,
  ) / 1000;
}
...
function mergeArchive(fresh, previous, previousUpdatedAt, now) {
  ...
  for (const video of fresh) {
    ...
    // A real 24-hour gain can never exceed the video's current lifetime views.
    // Capping the projection prevents very young uploads from showing impossible-looking values.
    const normalizedGain = Math.round(Math.min(currentViews, Math.max(0, gain24h)));
    byId.set(video.youtubeId, {
      ...old,
      ...video,
      views: currentViews,
  ...
  const rawRanked = sortVideos(newest, "trend", now);
  const channelAppearances = new Map();

  for (const video of rawRanked) {
    const appearance = channelAppearances.get(video.channel) ?? 0;
    channelAppearances.set(video.channel, appearance + 1);
    video.rawTrendScore = video.trendScore;
    const diversityWeight = channelDiversityWeights[appearance]
      ?? channelDiversityWeights.at(-1);
    video.trendScore = Math.round(video.trendScore * diversityWeight * 1000) / 1000;
  }

  const ranked = sortVideos(rawRanked, "trend", now);
