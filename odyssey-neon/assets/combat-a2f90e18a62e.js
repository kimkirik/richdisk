...
export const formationOffsets = (double) =>
  double >= 2 ? [-36, 0, 36] : double === 1 ? [-20, 20] : [0];
      ...
      : spec.id === "laser"
        ...
        : spec.id === "rocket"
          ...
          : level >= 3
            ? 2
            : 1;
  const power = spec.damage * (1 + level * 0.4) * (boost ? 1.25 : 1);
...
export function collectDrop(state, drop) {
  ...
  if (drop.kind === "M") {
    ...
    if (ship.double < 2) {
      ...
      announce(
        state,
        `${ship.double + 1}기 합체! · 모든 기체가 함께 발사`,
        "#ffd86a",
      );
    } else {
...
export function drawCombatOverlay(ctx, state) {
  ctx.save();
  const boosted = state.driveTicks > 0;
  ctx.fillStyle = "#081327dd";
  ctx.fillRect(200, 887, 400, 49);
  ctx.fillStyle = "#263446";
  ctx.fillRect(218, 920, 364, 5);
  ctx.fillStyle = boosted ? "#ffd86a" : "#55f7ff";
  ctx.fillRect(
    218,
    920,
    364 * (boosted ? state.driveTicks / 360 : (state.driveCharge || 0) / 12),
    5,
  );
  ...
  ctx.fillText(
    boosted
      ? `OVERDRIVE · ${Math.ceil(state.driveTicks / 60)}초`
      : `연속 격추 ${state.driveCharge || 0}/12 · OVERDRIVE`,
    400,
    910,
  );
  ctx.restore();
