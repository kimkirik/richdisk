...
export const formationOffsets = (double) => (double === 1 ? [-25, 25] : [0]);

export const shipGeometry = (double) => ({
  scale: double >= 2 ? 1.25 : 0.7,
  marginX: double >= 2 ? 62 : double === 1 ? 59 : 34,
  bottomMargin: double >= 2 ? 78 : 48,
  hitRadius: double >= 2 ? 17 : 11,
});
...
export function fireVolley(state) {
  ...
  for (let barrel = 0; barrel < count; barrel++) {
    for (const offset of offsets) {
      ...
      const y = ship.y - (fused ? 54 : 29) + (offset ? 4 : 0);
      result.push({
        x,
        y,
        prevX: x,
